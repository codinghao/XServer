...
private:
    ...
    class DelegateImpl<std::pair<T, TFunctor>> : public DelegateImplBase
    {
    public:
        DelegateImpl(std::pair<T, TFunctor>& _functor)
            : m_Functor(_functor)
        {}

        DelegateImplBase* Clone()
        {
            return new DelegateImpl<std::pair<T, TFunctor>>(m_Functor);
        }
